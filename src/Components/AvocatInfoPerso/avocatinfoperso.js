...
const AvocatInfoPerson = () => {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      const response = await axios.put('URL_DE_VOTRE_API_DJANGO_MODIFIER', formData);
      setAvocat(response.data);
      setOriginalAvocat(response.data);
      setIsEditing(false);
    } catch (error) {
      ...
      <div className='avocatinfoperso'>
        ...
        {isEditing && (
          <div className='modifier-form-container'>
            <div className='modifier-form'>
              ...
              <input type='text' value={avocat.nom} onChange={(e) => setAvocat({ ...avocat, nom: e.target.value })} />
              <label>Nouvelle adresse :</label>
              <input type='text' value={avocat.adresse} onChange={(e) => setAvocat({ ...avocat, adresse: e.target.value })} />
              <label>Nouvelle spécialité :</label>
              <input type='text' value={avocat.specialite} onChange={(e) => setAvocat({ ...avocat, specialite: e.target.value })} />
              <div className='button-container'>
                <div className='enregistrer'><button onClick={() => handleFormSubmit(avocat)}>Enregistrer</button></div>
                <div className='annuler'><button onClick={handleAnnulerClick}>Annuler</button></div>
              </div>
            </div>
          </div>
        )}
        <div className='infoperso'>
          ...
          <div className='modifier'>
            <button id='modifierButton' onClick={handleModifierClick}>Modifier</button>
          </div>
        </div>
      </div>
    </div>
